...
def setup_Shades(en_idf, enng, shaded_mats, verbose=True):
    ...
    ShadWinNodes   = [n for n in enng.nodes if hasattr(n, 'mat_def') and n.mat_def in shaded_mats.keys()]
    if verbose:
    ...
    for ShadWinNode in ShadWinNodes: # we go through each of them
        ...
        if len(DATA["Faces"])==0:    continue # silently drop unwanted shades # NOTE: be aware that unexistent materials,
        ...
        ##### ANPASSEN: wenn zwei(oder mehr) verschiedene Materialien auf das selbe Shading-Controll zugreifen, muss jedes davon dafuer neu erzeugt werden. => auch CONTROL anpassen
        ##### WORKAROUND BISHER: Die entsprechenden Shading-Control-Nodes ein mal pro ShadedWindows-Node-Material kopieren.
        ConstName = "{}__{}__{}".format(DATA["Material"], ShadeName, CONTROL["Placement"])
        if ConstName not in ShadConstrs:
            constr = shaded_mats[DATA["Material"]]
        ...
        if ContrName not in ContrWritten:
            ...
            contr_text = epentry("WindowProperty:ShadingControl", params, paramsvs)
            ContrWritten.append(ContrName)
        else: contr_text = ""
        
        ### preparing Faces
        if verbose: print("PATCH: ... preparing faces")
        for faceID in DATA["Faces"]: ShadedWindows[faceID] = ContrName

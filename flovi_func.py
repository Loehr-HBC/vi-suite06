...
flovi_p_bounds = {'if': {'0': ('zeroGradient', 'fixedValue', 'totalPressure',
        'inletOutlet', 'pressureInletOutletVelocity', 'PressureInletVelocity '),
        '1': ('zeroGradient', 'fixedValue'),    '2': ['None']},
    'sf': {'0': ('zeroGradient', 'fixedValue', 'freestreamPressure', 'totalPressure'),
        '1': ['zeroGradient'],                  '2': ['None']},
    'bsf': {'0': ('calculated', 'calculated', 'calculated', 'calculated'),
        '1': ('calculated', 'calculated'),      '2': ('None',)},
    'bbsf': {'0': ('calculated', 'calculated', 'calculated', 'calculated'),
        '1': ('calculated', 'calculated'),      '2': ('None',)}}

flovi_u_bounds = {'icoFoam': {'0': ('zeroGradient','noSlip', 'fixedValue',
        'inletOutlet'), '1': ('zeroGradient', 'noSlip', 'fixedValue'), '2': ['None']},
    'simpleFoam': {'0': ('zeroGradient', 'fixedValue', 'inletOutlet', 'freestream',
        'pressureInletOutletVelocity', 'slip'), '1': ('noSlip', 'fixedValue', 'slip'),
        '2': ['None']},
    'buoyantSimpleFoam': {'0': ('zeroGradient', 'fixedValue', 'inletOutlet',
        'freestream', 'pressureInletOutletVelocity', 'slip'),
...
flovi_nutilda_bounds = {'simpleFoam': {'0': ('zeroGradient', 'fixedValue'),
                          '1': ('zeroGradient', 'fixedValue'), '2': ['None']},
    'buoyantSimpleFoam': {'0': ('zeroGradient', 'fixedValue'),
                          '1': ('zeroGradient', 'fixedValue'), '2': ['None']}}

flovi_k_bounds = {'simpleFoam': {'0': ('fixedValue', 'inletOutlet'),
                                 '1': ['kqRWallFunction'], '2': ['None']},
    'buoyantSimpleFoam': {'0': ('fixedValue', 'inletOutlet',
                                'turbulentIntensityKineticEnergyInlet'),
                                 '1': ['kqRWallFunction'], '2': ['None']}}

flovi_epsilon_bounds = {'simpleFoam': {'0': ('fixedValue', 'inletOutlet'),
                                       '1': ['epsilonWallFunction'], '2': ('None')},
                        'buoyantSimpleFoam': {'0': ('fixedValue', 'inletOutlet'),
                                       '1': ['epsilonWallFunction'], '2': ['None']}}

flovi_omega_bounds = {'simpleFoam': {'0': ('zeroGradient', 'fixedValue'),
                                     '1': ['omegaWallFunction'], '2': ['None']},
                        'buoyantSimpleFoam': {'0': ('zeroGradient', 'fixedValue'),
                                    '1': ['omegaWallFunction'], '2': ['None']}}

flovi_t_bounds = {'buoyantSimpleFoam': {'0': ('zeroGradient', 'fixedValue', 'inletOutlet'),
                                        '1': ('zeroGradient', 'fixedValue'), '2': ['None']}}

flovi_prgh_bounds = {'buoyantSimpleFoam': {'0': ('fixedFluxPressure',
        'prghTotalHydrostaticPressure', 'fixedValue', 'prghPressure'),
        '1': ('fixedFluxPressure', 'fixedValue'), '2': ['None']}}
...
def ret_fvbp_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_p_bounds[context.scene.vi_params['flparams']['solver_type']][mat.flovi_bmb_type]]

def ret_fvbu_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_u_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]

def ret_fvbnut_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_nut_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]

def ret_fvbnutilda_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_nutilda_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]

def ret_fvbk_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_k_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]

def ret_fvbepsilon_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_epsilon_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]

def ret_fvbomega_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_omega_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]

def ret_fvbt_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_t_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]

def ret_fvba_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_a_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]

def ret_fvbprgh_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_prgh_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]

def ret_fvrad_menu(mat, context):
    return [('{}'.format(b), '{}'.format(b), '{} boundary type'.format(b))
        for b in flovi_rad_bounds[context.scene.vi_params['flparams']['solver']][mat.flovi_bmb_type]]
...
def fvboundwrite(o):
    ...
    for mat in o.data.materials:
        boundary += "  {}\n  {{\n    type {};\n    faces\n    (\n".format(mat.name,
                ("patch", "wall", "symmetry", "empty")[int(mat.flovi_bmb_type)])#;\n\n"
...
def fvblbmgen(mats, ffile, vfile, bfile, meshtype):
    ...
    if bobs:
        ...
        bpy.ops.object.add(type='MESH', layers=(i==19 for i in range(20)))
...
def fvvarwrite(scene, obs, node):
    ...
    (Uentry, nutildaentry, nutentry, kentry, eentry, oentry, tentry, p_rghentry, aentry, Gentry) = [
        "dimensions [{} {} {} {} 0 0 0];\ninternalField   uniform {};\n\nboundaryField\n{{\n".format(*var)
        for var in (
...
def fvcdwrite(solver, st, dt, et):
    ...
    cdict = {'application': solver, 'startFrom': 'startTime',
            'startTime': '{}'.format(st), 'stopAt': 'endTime',
            'endTime': '{}'.format(et), 'deltaT': '{:.5f}'.format(dt),
            'writeControl': 'timeStep', 'writeInterval': '1',
            'purgeWrite': '{}'.format(pw), 'writeFormat': 'ascii',

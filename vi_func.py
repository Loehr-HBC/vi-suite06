...
def retvpvloc(context):
    return bpy_extras.view3d_utils.region_2d_to_origin_3d(context.region,
        context.space_data.region_3d, (context.region.width/2.0, context.region.height/2.0))
...
def lividisplay(self, scene):
    ...
    if self.id_data.name in svp['liparams']['livir']:
        ...
        if len(frames) > 1:
            ...
            lms = {fi: self.id_data.data.animation_data.action.fcurves.new(
                data_path='polygons[{}].material_index'.format(fi)) for fi in fis}
